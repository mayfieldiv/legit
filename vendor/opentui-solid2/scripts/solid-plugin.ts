...
export function createSolidTransformPlugin(
  input: CreateSolidTransformPluginOptions = {},
): BunPlugin {
  return {
    ...
    setup: (build) => {
      build.onResolve({ filter: /^solid-js$/ }, () => ({
        path: "solid-js",
        namespace: "opentui-solid-runtime",
      }));

      build.onLoad({ filter: /^solid-js$/, namespace: "opentui-solid-runtime" }, () => ({
        contents: 'export * from "solid-js/dist/solid.js"',
        loader: "js",
      }));
