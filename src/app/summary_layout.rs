//! Complete layout derivation for the selected PR's summary panel. Produces,
//! top to bottom:
...
//!
//! Pure derivation with no painting: `view::summary` renders the lines while
//! `update` measures them to clamp the scroll offset. Keeping the shared
//! derivation in the app layer prevents the reducer from depending on view
//! modules and makes the rendered content the single source of scroll height.
...
use super::model::{FilesState, Model};

/// Scroll intent for the summary currently associated with one selected PR.
/// Bundling the owner and offset makes it impossible to update one without
/// going through the invariant-preserving interface below.
...
impl SummaryState {
    /// Lines scrolled past the top of the current summary.
    pub(crate) fn offset(&self) -> usize {
        self.offset
    }

    /// Reset to the top when the summary starts showing a different PR.
    /// Returns whether the selected identity changed.
    pub(crate) fn sync_pr(&mut self, key: Option<PrKey>) -> bool {
        if self.pr == key {
            return false;
        }
        self.pr = key;
