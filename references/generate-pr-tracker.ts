...
if (!ghToken) {
  try {
    const raw = execSync(
      'security find-generic-password -s "gh:github.com" -a "" -w',
      { encoding: "utf-8" }
    ).trim();
...
const allPrs: Array<{
  ...
  gh(
    ...
    "--state", "open",
    "--limit", "200",
    "--json", "number,title,author,createdAt,additions,deletions,reviewDecision,isDraft"
  )
);
...
const DRAFT_OWNERS: Record<string, string> = {
  dkattan: "Darren",
  colinblaise: "Colin",
  mayfieldiv: "Mayfield",
};
...
[
  approved, needsReviewSmall, needsReviewMedium, needsReviewLarge,
  changesRequested, draftDarren, draftColin, draftMayfield, draftBot,
  draftOthers, stale,
].forEach((arr) => arr.sort(desc));

const totalDrafts =
  draftDarren.length + draftColin.length + draftMayfield.length +
  draftBot.length + draftOthers.length;
const totalReview =
  needsReviewSmall.length + needsReviewMedium.length + needsReviewLarge.length;
const conflictCount = [...meta.values()].filter(
  (m) => m.mergeable === "CONFLICTING"
).length;
